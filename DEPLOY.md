...
## Сигнал через MacroDroid (бета)
На главном экране у пользователя есть ползунок **«Отработать по сигналу»**. Если
он включён, то при «сигнале» все активные устройства этого пользователя
запускаются в работу автоматически (и потом присылают отчёт). «Сигнал» — это
приход SMS со словом **«символ»** на ваш телефон-монитор с MacroDroid.

**Настраивать нужно ПОСЛЕ деплоя сервера** (нужен рабочий адрес и секрет).

1. Узнайте адрес вебхука: он печатается в логах при старте —
   ```bash
   journalctl -u alfa-sms | grep "Signal webhook"
   ```
   Получите строку вида `https://project.alfa-vpn.ru/api/signal/<СЕКРЕТ>`.
   (Секрет можно задать заранее в `.env` через `SIGNAL_SECRET`.)
2. В приложении **MacroDroid** на телефоне-мониторе создайте макрос:
   - **Триггер:** «Получено SMS» → содержит текст `символ`.
   - **Действие:** «HTTP-запрос (GET)» → вставьте адрес вебхука из п.1.
   - Сохраните и включите макрос.
3. Теперь при приходе «символ» сервер мгновенно запускает работу у всех, кто
   включил «Отработать по сигналу». Это бета — рекомендуйте пользователям всё
   равно настраивать расписание.

## Логика работы, сигнала и расписания (важно)
- **Номера:** платёжные SMS уходят на **7878**; оттуда же приходят «Операция
  отклонена» и «Оплата не произведена». «символ»/«успешно» приходят с **8464**,
  и «Ок» уходит на 8464. (Меняется через `RECIPIENT_NUMBER` / `SIGNAL_NUMBER`.)
- **«Ок» на «символ» — всегда**, включена система или нет (не теряем возможность).
- **Расписание запускается само** и имеет приоритет. Сервер сам включает работу
  в начале окна (и каждый день при «повторять»). Часовой пояс окон — по `TZ`
  сервера (обязательно задайте его в `.env`).
- **Сигнал** (MacroDroid → вебхук) запускает работу только у тех, кто включил
  «Отработать по сигналу», **и только если сейчас НЕ в окне расписания** и работа
  ещё не идёт. Если время совпало с расписанием — сигнал игнорируется,
  отрабатывает расписание.
- **Что делает сигнал на устройстве:** шлёт первый платёж 3 раза с интервалом
  1 c; нет «символ» — прекращает; есть «символ» → «Ок» и дальше по остальным
  реквизитам обычным порядком.
- **Остановка + отчёт:** когда каждое активное устройство завершило работу
  (получило «Оплата не произведена» с 7878 или отработало все платежи), сессия
  закрывается и бот присылает финальный отчёт.

## Уведомление об отклонённом реквизите
Если в ответ на платёж с номера 7878 приходит SMS со словами «Операция
отклонена…», приложение сообщает об этом серверу, и бот присылает владельцу
устройства сообщение: «❗️ Реквизит «…» отклоняется платёжным шлюзом — замените
его на другой. Устройство: «…».» Ничего настраивать не нужно — работает само.
