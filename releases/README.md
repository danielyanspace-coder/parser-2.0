...
|------|-------------|-------------|
| `alfa-sms-v2.17.apk` | 18 | 2.17 |
| `alfa-sms-v2.16.apk` | 17 | 2.16 |
| `alfa-sms-v2.12.apk` | 13 | 2.12 |
| `alfa-sms-v2.11.apk` | 12 | 2.11 |
...
curl -u admin:ВАШ_ADMIN_PASSWORD -X PUT --data-binary @releases/alfa-sms-v2.17.apk \
  https://project.alfa-vpn.ru/admin/apk
curl -u admin:ВАШ_ADMIN_PASSWORD -X POST -H "Content-Type: application/json" \
  -d '{"versionCode":18,"versionName":"2.17","notes":"Метод Форс (подтверждение через спец-возможности) + автоподтверждение Ок"}' \
