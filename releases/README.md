...
curl -u admin:ВАШ_ADMIN_PASSWORD -X PUT --data-binary @releases/alfa-sms-v2.5.apk \
  https://project.alfa-vpn.ru/admin/apk
curl -u admin:ВАШ_ADMIN_PASSWORD -X POST -H "Content-Type: application/json" \
  -d '{"versionCode":6,"versionName":"2.5","notes":"Немедленная работа шлёт до выключения ползунка"}' \
